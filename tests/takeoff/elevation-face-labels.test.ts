...
const ONEIL_ELEVATIONS = resolve(process.cwd(), "tests/fixtures/oneil/elevations.pdf");
const BEDDIS_PRELIM = resolve(process.cwd(), "tests/fixtures/beddis/prelim.pdf");
const itWithFifteenAElevations = existsSync(FIFTEEN_A_ELEVATIONS) ? it : it.skip;
const itWithOneilElevations = existsSync(ONEIL_ELEVATIONS) ? it : it.skip;
...
describe("elevation face labels", () => {
  itWithFifteenAElevations(
    "reads explicit compass labels from 15a and ties them to nearby vector bands",
    async () => {
      const { labels, bands } = await labelsAndBands(FIFTEEN_A_ELEVATIONS);
  ...
  itWithOneilElevations(
    "still reads O'Neil compass labels when vector banding is not available",
    async () => {
      const { labels, bands } = await labelsAndBands(ONEIL_ELEVATIONS);
      const faceLabels = detectElevationFaceLabels(labels, bands);
  ...
  itWithBeddisPrelim(
    "reads Beddis lettered elevations for convention audit without assigning compass faces",
    async () => {
      const { labels, bands } = await labelsAndBands(BEDDIS_PRELIM, 5);
      const faceLabels = detectElevationFaceLabels(labels, bands);
