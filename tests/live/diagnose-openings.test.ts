...
describe.skipIf(!LIVE)("LIVE DIAG openings (report-only)", () => {
  it("JM-0027 / JM-0029 vs control JM-0020 — window pipeline state", async () => {
    for (const jn of ["JM-0027", "JM-0029", "JM-0020"]) {
      ...
      if (!job) { console.log(`[diag] ${jn}: NOT FOUND (err=${trunc(jobs.error)})`); continue; }
      ...
      if (tj) {
        ...
        for (const k of ["window_count", "windows_source", "source", "windows_by_room", "windows_schedule", "openings", "extraction_errors", "errors", "vision_meta"]) {
