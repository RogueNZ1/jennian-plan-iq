...
async function main() {
  ...
  for (const k of ["singles", "doubles", "cavitySliders", "barn"] as const) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- pdf.js/Supabase boundary types are deliberately loose here
    const got = (result.counts as any)[k],
      want = bench.expected[k === "cavitySliders" ? "cavitySliders" : k];
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions -- grandfathered at lint-gate introduction
  ...
  result.flags.length === (bench.expected.flags ?? 0)
    ...
    : bad(
        `flags: got ${result.flags.length} (${result.flags.map((f) => `${f.widthMm}@${f.x | 0},${f.y | 0} ${f.note ?? ""}`).join("; ")}), want ${bench.expected.flags ?? 0}`,
      );
  ...
  for (const gt of bench.doors) {
    ...
    if (hit) {
      matched.add(hit);
      ok(`${gt.label} (${gt.type} ${gt.widthMm})`);
    } else bad(`${gt.label} (${gt.type} ${gt.widthMm} near ${gt.near}) — not found`);
  }

  console.log("\n── exclusions ──");
  for (const ex of bench.mustNotAppear) {
    const leak = [...all, ...result.flags].find(
      (h) => Math.hypot(h.x - ex.near[0], h.y - ex.near[1]) < 30 && h.widthMm === ex.widthMm,
    );
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions -- grandfathered at lint-gate introduction
  ...
  extras.length === 0
    ...
    : extras.forEach((e) =>
        bad(`extra ${e.type} ${e.widthMm} @ (${e.x | 0},${e.y | 0}) ${e.note ?? ""}`),
      );
  ...
  console.log(
    `QS export: H187 singles=${result.counts.singles}  H192 doubles=${result.counts.doubles}  H193 cavity=${result.counts.cavitySliders}  barn=${result.counts.barn}`,
  );
