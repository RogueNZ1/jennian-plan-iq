...
const mockJob: Row = {
  id: "job-1", job_number: "JM-0099", client_name: "Test Client", address: "1 Test St",
  template: null, plan_type: "concept", status: "active", created_by: "u", created_at: "",
  updated_at: "", floor_area_m2: 100, perimeter_lm: 40, smw_enabled: false,
  plan_type_override: null, confidence_score: null, jmw_number: "JM-0099",
  client_first_name: "Test", client_surname: "Client", street_address: "1 Test St",
  address_line2: null, city: "Feilding", email: null, phone: null,
};
...
vi.mock("../../src/integrations/supabase/client", () => ({
  supabase: {
    from: vi.fn((table: string) => {
      if (table === "jobs") {
        return { select: () => ({ eq: () => ({ single: () => Promise.resolve({ data: mockJob, error: null }) }) }) };
      }
      if (table === "door_counts") {
        return { select: () => ({ eq: () => ({ maybeSingle: () => Promise.resolve({ data: db.doorCounts, error: null }) }) }) };
      }
      if (table === "module_items") {
        return { select: () => ({ eq: () => Promise.resolve({ data: db.items, error: null }) }) };
      }
      if (table === "opening_schedule") {
        return { select: () => ({ eq: () => Promise.resolve({ data: db.openings, error: null }) }) };
      }
...
const door = (qty: number, opening_type = "internal_door"): Row => ({
  id: `o-${Math.random()}`, job_id: "job-1", opening_type, room_name: "Hall",
  quantity: qty, width_mm: 810, height_mm: 1980,
});
...
describe("AUDIT §5.5 — internal doors reach the export", () => {
  ...
  it("CONFIRMED door_counts still override the schedule fallback (precedence unchanged)", async () => {
    db.openings = [door(8)];
    db.doorCounts = { confirmed_at: "2026-06-01", standard: 6, double_doors: 1, cavity_sliders: 2, barn_sliders: 0 };
    const data = await buildQSExportData("job-1");
  ...
  it("door-labelled module items still override the schedule fallback (precedence unchanged)", async () => {
    db.openings = [door(8)];
    db.items = [{ module_id: "m", label: "Interior door — standard hinged", extracted_value: "7", approved_value: null, unit: "count", value_source: "extracted" }];
    const data = await buildQSExportData("job-1");
  ...
  it("windows and garage doors are untouched by the filter change", async () => {
    db.openings = [
      door(5),
      { id: "w1", job_id: "job-1", opening_type: "window", room_name: "Bed 1", quantity: 2, width_mm: 1800, height_mm: 1300 },
      { id: "g1", job_id: "job-1", opening_type: "garage_door", room_name: "Garage", quantity: 1, width_mm: 4800, height_mm: 2100 },
    ];
    const data = await buildQSExportData("job-1");
