...
const FIXTURES = [
  {
    ...
    criticalRows: [
      ...
      (os: Opening[]) =>
        expect(
          os.some(
            (o) => o.type === "sectional_door" && !o.glazed && Math.abs(o.width_m - 2.7) < 0.05,
          ),
        ).toBe(true),
  ...
  },
  {
    ...
    criticalRows: [
      (os: Opening[]) =>
        expect(
          os.filter((o) => ["window", "slider", "garage_window"].includes(o.type)),
        ).toHaveLength(13),
      (os: Opening[]) =>
        expect(
          os.some(
            (o) => o.type === "sectional_door" && Math.abs(o.width_m - 4.8) < 0.05 && !o.glazed,
          ),
        ).toBe(true),
    ],
  },
  {
    ...
    criticalRows: [
      ...
      (os: Opening[]) =>
        expect(os.some((o) => o.type === "sectional_door" && !o.glazed)).toBe(true),
    ],
  },
  {
    id: "oneil",
    ...
    criticalRows: [
      ...
      (os: Opening[]) =>
        expect(
          os.some((o) => o.type === "pa_door" && Math.abs(o.width_m - 0.96) < 0.05 && o.glazed),
        ).toBe(true),
    ],
  },
  {
    ...
    criticalRows: [
      (os: Opening[]) =>
        expect(
          os.some(
            (o) =>
              o.type === "slider" &&
              (o.room ?? "").toLowerCase().includes("din") &&
              Math.abs(o.width_m - 2.4) < 0.05 &&
              o.glazed,
          ),
        ).toBe(true),
      ...
      (os: Opening[]) =>
        expect(os.some((o) => o.type === "window" && (o.room ?? "").toLowerCase() === "wc")).toBe(
          true,
        ),
    ],
  },
  ...
  for (const fixture of FIXTURES) {
    it(`${fixture.id}: all openings appear as flat rows, critical types present`, () => {
      ...
      if ((fixture as { useLiveSnapshot?: boolean }).useLiveSnapshot) {
        ...
        const gt = JSON.parse(
          readFileSync(resolve(FIX, `${fixture.id}/ground-truth.json`), "utf8"),
        );
