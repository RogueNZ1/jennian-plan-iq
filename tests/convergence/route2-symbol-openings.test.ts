...
describe("Route 2 — foldSymbolOpenings", () => {
  ...
  it("15A-shaped: folds 4 callouts, asserts heights, reconciles the garbled garage", () => {
    ...
    const syms = [
      sym("sectional_door", 2700),
      sym("garage_window", 2000),
      sym("slider", 2000),
      sym("entrance", 1030),
    ];
    ...
    expect(byType(r.openings, "slider")).toMatchObject({
      width_m: 2,
      height_m: 2.1,
      glazed: true,
      source: "callout",
    });
    expect(byType(r.openings, "entrance")).toMatchObject({
      width_m: 1.03,
      height_m: 2.1,
      glazed: true,
    });
