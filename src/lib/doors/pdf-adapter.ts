...
export async function extractPageGeometry(page: PdfPage): Promise<PageGeometry> {
  ...
  type Tok = { s: string; x: number; y: number; w: number; h: number; fs: number; vertical: boolean };
  ...
  for (const it of tc.items) {
    ...
    toks.push({ s: it.str, x: px, y: py, w: vertical ? 0 : adv, h: vertical ? adv : 0, fs, vertical });
  }
  ...
  for (const t of toks) {
    ...
    if (!run) { run = { toks: [], vertical: t.vertical, line: lineCoord }; runs.push(run); }
    run.toks.push(t);
  ...
  for (const run of runs) {
    ...
    for (let i = 0; i < run.toks.length; i++) {
      const t = run.toks[i];
      if (cur.length === 0) { cur.push(t); text = t.s; continue; }
      ...
      if (gap < 0.70 * fs) { cur.push(t); text += t.s; }
      else if (gap < 1.45 * fs) { cur.push(t); text += " " + t.s; }
      else { flush(); cur.push(t); text = t.s; }
    }
    flush();
  }
  ...
  const flushPoly = () => { if (poly.length >= 3) polylines.push(poly); poly = []; };
  const emitLine = (ax: number, ay: number, bx: number, by: number) => {
    const a = toPagePt(ax, ay), b = toPagePt(bx, by);
  ...
  for (let i = 0; i < ol.fnArray.length; i++) {
    ...
    if (fn === OPS.save) stack.push(ctm);
      ...
      let k = 0, cx = 0, cy = 0;
      for (const op of ops) {
        if (op === OPS.moveTo) { flushPoly(); cx = co[k++]; cy = co[k++]; }
        else if (op === OPS.lineTo) { const nx = co[k++], ny = co[k++]; emitLine(cx, cy, nx, ny); cx = nx; cy = ny; }
        else if (op === OPS.curveTo) { k += 4; cx = co[k++]; cy = co[k++]; } // beziers irrelevant here (Qt emits polylines)
        else if (op === OPS.curveTo2 || op === OPS.curveTo3) { k += 2; cx = co[k++]; cy = co[k++]; }
        else if (op === OPS.rectangle) { k += 4; } // walls/fills — not arc material
        else if (op === OPS.closePath) { flushPoly(); }
      }
      flushPoly();
    }
  }
