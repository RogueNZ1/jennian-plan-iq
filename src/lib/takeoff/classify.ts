...
const SPEC_INDICATORS: RegExp[] = [
  /preliminary\s*&\s*general/i,
  /\bfoundation\b/i,
  /\bexterior\b/i,
  /aluminium\s+joinery/i,
  /\binsulation\b/i,
  /interior\s+linings?/i,
  /\belectrical\b/i,
  /\blighting\b/i,
  /\bplumbing\b/i,
  /\bkitchen\b/i,
  /\blaundry\b/i,
  /\bexclusions?\b/i,
  /included\s+in\s+cost/i,
  /no\s+allowance/i,
  /roof\s+pitch/i,
  /stud\s+height/i,
  /area\s+over\s+frame/i,
  /\bperimeter\b/i,
  /garage\s+door/i,
  /cladding\s+type/i,
  /wall\s+linings?/i,
...
export function classifyPageWithType(
  ...
  if (fileType === "specification") {
    ...
    else if (/interior\s+linings?|\bskirting|\bscotia|architraves?/i.test(p.text))
      subtype = "linings";
    ...
    return {
      ...
      reason: reason(
        `Specification — ${subtype} (file_type=specification, ${specCount} indicators)`,
      ),
    };
  }
  ...
  if (has("cover sheet") || has("title sheet") || has("drawing index")) {
    return {
      ...
      pageType: "Cover Page",
      confidence: "high",
      reason: reason("cover/title keywords"),
    };
  }
  if (has("specification") || has("specifications") || has("m3 schedule") || has("master spec")) {
    return {
      ...
      pageType: "Specification",
      confidence: "high",
      reason: reason("specification keywords"),
    };
  }
  if (has("schedule") && (has("door") || has("window") || has("joinery") || has("opening"))) {
    return {
      ...
      pageType: "Schedule",
      confidence: "high",
      reason: reason("schedule keywords"),
    };
  }
  if (has("site plan") || has("locality plan") || has("title plan") || has("boundary")) {
    return {
      ...
      pageType: "Site Plan",
      confidence: "high",
      reason: reason("site keywords"),
    };
  }
  if (has("electrical") || has("lighting plan") || has("power plan")) {
    return {
      pageNumber: p.pageNumber,
      pageType: "Electrical Plan",
      confidence: "high",
      reason: reason("electrical keywords"),
    };
  }
  if (has("plumbing") || has("drainage") || has("waste plan")) {
    return {
      pageNumber: p.pageNumber,
      pageType: "Plumbing Plan",
      confidence: "high",
      reason: reason("plumbing keywords"),
    };
  }
  ...
  if (/\bsection\s+[a-z0-9]/i.test(p.text) || has("cross section") || has("long section")) {
    return {
      ...
      pageType: "Sections",
      confidence: "high",
      reason: reason("section keywords"),
    };
  }
  if (has("elevation")) {
    return {
      ...
      pageType: "Elevations",
      confidence: "mid",
      reason: reason("elevation keywords"),
    };
  }
  ...
  if (dimHits >= 18) {
    return {
      ...
      pageType: "Dimension Floorplan",
      confidence: "low",
      reason: reason(`${dimHits} dim-like numbers, no title`),
    };
  }
  return {
    pageNumber: p.pageNumber,
    pageType: "Unknown",
    confidence: "low",
    reason: reason("no recognised keywords"),
  };
}
...
export function normaliseRoomName(raw: string): string {
  ...
  if (
    s.includes("family") ||
    s.includes("living") ||
    s.includes("lounge/dining") ||
    s.includes("open plan")
  )
  ...
  if (s.includes("laundry") || s.includes(" wm") || s === "wm" || s.includes("utility"))
    return "Laundry";
