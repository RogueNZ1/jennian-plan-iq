  ...
  useEffect(() => {
    async function load() {
      ...
      if (jobs) {
        ...
        for (const j of jobs) {
          const key = new Date(j.created_at).toLocaleDateString("en-NZ", { month: "short", year: "2-digit" });
    ...
    <AppLayout>
      <div className="px-8 py-8 max-w-6xl">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="text-[13px] font-semibold tracking-tight mb-4">Jobs Uploaded — Last 6 Months</div>
            <ResponsiveContainer width="100%" height={220}>
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="mt-5 text-[34px] font-semibold tracking-tight leading-none tabular-nums">{value}</div>
    </div>
  );
}
