    ...
    <AppLayout>
      <div className="px-8 py-8 max-w-7xl">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <Select
            value={filterRole}
            onChange={(v) => setFilterRole(v as "all" | AppRole)}
            options={[
              { v: "all", label: "All roles" },
              ...ROLES.map((r) => ({ v: r, label: ROLE_LABEL[r] })),
            ]}
          />
          <Select
            value={filterStatus}
                      ...
                      <div className="flex items-center justify-end gap-1">
                        {r.kind === "profile" ? (
                          <>
                            ...
                            <IconBtn
                              ...
                              onClick={() =>
                                setDeleteTarget({
                                  id: r.id,
                                  label: r.email || r.name || "this user",
                                })
                              }
                              disabled={!canManage || r.id === user?.id}
                            >
                          ...
                          <>
                            <IconBtn
                              title={
                                canInvite ? "Resend invite" : "Only the Owner can resend invites"
                              }
                            ...
                            <IconBtn
                              title={
                                canInvite ? "Cancel invite" : "Only the Owner can cancel invites"
                              }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently deletes <span className="font-medium">{deleteTarget?.label}</span> —
              their account, role and profile are removed and they can no longer sign in. This can't
              be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function InviteModal({
  ...
  onClose: () => void;
  onCreated: (p: InvitePayload) => Promise<void> | void;
}) {
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div className="sm:col-span-2">
          <Field
            ...
            value={email}
            onChange={setEmail}
            placeholder="name@jennian.co.nz"
            type="email"
          />
        </div>
        <div>
        ...
        <div className="sm:col-span-2">
          <Label>
            Welcome message <span className="text-muted-foreground/60 text-[10px]">(optional)</span>
          </Label>
          <textarea
            value={welcome}
...
function RoleModal({
  ...
  row: UserListRow;
  onClose: () => void;
  onSave: (r: AppRole) => Promise<void> | void;
}) {
        ...
        <ul className="divide-y divide-border">
          {rows.map((r) => {
            const actor = r.actor_user_id
              ? (profileById[r.actor_user_id]?.full_name ??
                profileById[r.actor_user_id]?.email ??
                "Unknown")
              : "System";
            return (
