          ...
          {IQ_MODULES.map((m) => {
            ...
            return (
              ...
              >
                <svg
                  viewBox="0 0 80 80"
                  className="absolute -top-4 -right-4 w-20 h-20 text-foreground/[0.04] pointer-events-none"
                  aria-hidden
                >
                  <path
                    d="M10 70 V20 L40 6 L70 20 V70"
                    stroke="currentColor"
                    strokeWidth="1"
                    fill="none"
                  />
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  ...
                  <Metric
                    label="Confidence"
                    value={`${r?.confidence_avg ?? 0}%`}
                    accent={(r?.confidence_avg ?? 0) >= 80}
                  />
                </div>

                <div className="mt-3 text-[10px] text-muted-foreground">
                  {r?.last_run_at
                    ? `Updated ${new Date(r.last_run_at).toLocaleString()}`
                    : loading
                      ? "Loading…"
                      : "Not yet reviewed"}
...
function Metric({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
      ...
      <div
        className={`mt-0.5 text-[15px] font-semibold tabular-nums tracking-tight ${accent ? "text-primary" : ""}`}
      >
        {value}
      </div>
    </div>
...
export function StatusChip({ status }: { status: IQModuleStatus }) {
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10.5px] font-medium ${statusBadgeClass(status)}`}
    >
      {statusLabel(status)}
    </span>
  );
