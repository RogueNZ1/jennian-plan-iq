...
function Stat({
  ...
  return (
    <div className="group relative rounded-xl border border-border bg-card p-5 hover:border-primary/30 transition-colors shadow-[0_1px_0_rgba(0,0,0,0.02)]">
      {accent && (
        <span className="absolute left-0 top-4 bottom-4 w-[2px] rounded-r bg-primary/80" />
      )}
      <div className="flex items-start justify-between">
        ...
        <div
          className={`h-8 w-8 rounded-md grid place-items-center ${accent ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
        >
      ...
      <div className="mt-5 flex items-baseline gap-2">
        <div
          className={`text-[34px] font-semibold tracking-tight leading-none tabular-nums ${accent ? "text-primary" : ""}`}
        >
          {value}
        </div>
      </div>
...
function Dashboard() {
  ...
  const review = jobs.filter(
    (j) => j.status === "extracted" || j.status === "review_required",
  ).length;
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Latest plans reviewed in Jennian IQ.
              </p>
            </div>
            <Link
              to="/jobs"
                ...
                {recent.map((j) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-3 font-medium">
                      <Link to="/review" search={{ job: j.id }} className="hover:text-primary">
                        {j.job_number}
                      </Link>
                    </td>
