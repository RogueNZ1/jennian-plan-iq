...
- **Haydon's side (4 items):** (1) `RESEND_API_KEY` repo secret (same key 007 uses; from-address `iq@jennian.co.nz` needs jennian.co.nz verified in Resend) → branded email live on next deploy; (2) the DNS CNAME fix above; (3) paste the SQL (owner-only RLS migration + `alter table jobs add column if not exists specifications jsonb;`); (4) Supabase dashboard → disable public signups.
